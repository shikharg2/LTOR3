...
# Initialize Docker Swarm (must be a manager to create overlay networks)
...
if [ "$SWARM_STATE" = "active" ] && [ "$SWARM_MANAGER" = "true" ]; then
    echo "  Docker Swarm already active (manager node)."
elif [ "$SWARM_STATE" = "active" ] && [ "$SWARM_MANAGER" != "true" ]; then
    echo "  Node is a swarm worker, not a manager. Leaving old swarm..."
    docker swarm leave --force 2>/dev/null || true
    docker swarm init --advertise-addr 127.0.0.1
    echo "  Docker Swarm re-initialized as manager."
else
    docker swarm init --advertise-addr 127.0.0.1
...
# Pull PostgreSQL image used by the results database container
echo "  Pulling postgres:16-alpine image..."
docker pull postgres:16-alpine
echo "  postgres:16-alpine pulled successfully."
