...
class ConfigValidator:
    ...
    PROTOCOL_PARAMS = {
        "speed_test": {
            "required": ["target_url"],
            "optional": {"duration": int, "parallel_streams": int},
            "target_url_format": "host:port",
            "constraints": {
                "duration": {"min": 1},
                "parallel_streams": {"min": 1, "max": 128},
            },
        },
        "web_browsing": {
            "required": ["target_url"],
            "optional": {"disable_cache": bool, "parallel_browsing": bool},
    ...
    PROTOCOL_METRICS = {
        ...
        "streaming": [
            ...
            "est_bitrate_bps", "error_count", "total_rebuffer_time",
            "download_speed", "latency", "jitter"
        ],
        "voip_sipp": [
    ...
    def validate(self, config: dict, skip_time_checks: bool = False) -> list[str]:
        """
        Validate the entire configuration.

        Args:
            config: Configuration dictionary
            skip_time_checks: If True, skip validation of start_time being in the past
                              (useful when loading previously saved configs in the GUI)

        Returns:
            List of validation errors (empty if valid)
        """
        self._skip_time_checks = skip_time_checks
        errors = []
    ...
    def _validate_global_settings(self, settings: dict) -> list[str]:
        ...
        # name is optional but must be a non-empty string if present
        if "name" in settings:
            name = settings["name"]
            if not isinstance(name, str) or len(name.strip()) == 0:
                errors.append("global_settings: 'name' must be a non-empty string")
            elif len(name) > 255:
                errors.append("global_settings: 'name' must be 255 characters or fewer")
    ...
    def _validate_scenario(self, scenario: dict, index: int) -> list[str]:
        ...
        if protocol == "speed_test":
            ...
            if mode == "recurring":
                ...
                num_targets = len(parameters.get("target_url", []))
                estimated_test_time = estimate_speed_test_duration(duration_seconds) * max(num_targets, 1)
                ...
                if interval_seconds > 0 and estimated_test_time >= interval_seconds:
                    errors.append(
                        ...
                        f"(3x{duration_seconds}s + 25s overhead) x {max(num_targets, 1)} target(s)) must be less than "
    ...
    def _validate_schedule(self, schedule: dict, prefix: str) -> list[str]:
        ...
        if start_time is None:
            ...
            try:
                ...
                if not getattr(self, '_skip_time_checks', False) and parsed_time <= datetime.now(timezone.utc):
                    errors.append(
    ...
    def _validate_parameters(self, parameters: dict, protocol: str, prefix: str) -> list[str]:
            ...
            if param in parameters:
                if expected_type is int:
                    if not isinstance(parameters[param], (int, float)):
                        errors.append(
                            f"{prefix}: parameters: '{param}' must be a number"
                        )
                elif not isinstance(parameters[param], expected_type):
                    errors.append(
        ...
        for param, rules in constraints.items():
            ...
            if "max" in rules and isinstance(value, (int, float)):
                if value > rules["max"]:
                    errors.append(
                        f"{prefix}: parameters: '{param}' must be <= {rules['max']}, got {value}"
                    )
