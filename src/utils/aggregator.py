...
def get_raw_values_for_metric(scenario_id: str, metric_name: str) -> list[float]:
    """
    Get raw metric values for a specific metric in a scenario.
    Skips error sentinel values (-1).
    ...
    Returns:
        List of raw float values for the metric
    """
...
def get_aggregated_value(scenario_id: str, metric_name: str, aggregation: str) -> float:
    ...
    return metric_stats.get(aggregation, metric_stats.get("avg", -1))
