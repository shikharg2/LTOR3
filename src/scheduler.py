...
from src.utils.aggregator import get_aggregated_value, get_raw_values_for_metric, save_scenario_summary, parse_percentile_aggregation
from src.utils.unit_converter import normalize_for_comparison
from src.utils.error_logger import log_error, set_current_scenario
...
def _parse_start_time(start_time_str: str) -> datetime:
    """Parse start_time string to a timezone-aware UTC datetime.
    Naive datetimes are treated as UTC to match config_validator and orchestrate."""
...
class ScenarioScheduler:
    def __init__(self):
        ...
        self.scheduler = BackgroundScheduler(
            timezone='UTC',
            job_defaults={
                'misfire_grace_time': 600,  # 10 min grace time for missed execution 
    ...
    def schedule_scenario(self, scenario_id: str, scenario_config: dict) -> None:
        ...
        if mode == "once":
            if start_time == "immediate":
                ...
                trigger = DateTrigger(run_date=_parse_start_time(start_time))
    ...
    def _execute_test(self, scenario_id: str, scenario_config: dict) -> None:
        ...
        if not expectations:
            print(f"Skipping execution for scenario {scenario_id}: no expectations defined")
            return
    ...
    def _evaluate_expectations(self, run_id: str, scenario_id: str,
        ...
        for expectation in expectations:
            ...
            if scope == "per_iteration" and result_metrics_list:
                ...
                percentile_threshold = parse_percentile_aggregation(aggregation)

                if percentile_threshold is not None:
                    # Percentile aggregation: X% of data points must pass the test
                    raw_values = get_raw_values_for_metric(scenario_id, metric_name)
                    if not raw_values:
                        status = "ERROR"
                        measured_display = "-1"
                    else:
                        ...
                        for val in raw_values:
                            m_norm, e_norm = normalize_for_comparison(
                                val, expected_value, expected_unit, metric_name
                            )
                            if self._compare_values(m_norm, operator, e_norm) == "PASS":
                                pass_count += 1
                        pass_rate = (pass_count / len(raw_values)) * 100
                        status = "PASS" if pass_rate >= percentile_threshold else "FAIL"
                        measured_display = f"{pass_rate:.1f}% passed ({pass_count}/{len(raw_values)})"
                    ...
                    # Standard aggregations (avg, min, max, stddev)
    ...
    def _compare_values(self, measured: float, operator: str, expected: float) -> str:
        ...
        comparisons = {
            ...
            "eq": abs(measured - expected) <= max(1e-9, 1e-6 * abs(expected)),
            "neq": abs(measured - expected) > max(1e-9, 1e-6 * abs(expected)),
        }
    ...
    def finalize_scenario(self, scenario_id: str) -> None:
        ...
        from src.utils.db import get_connection, insert_test_run
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if row:
                    ...
                    # No test runs exist — insert a synthetic run so the FK
                    # constraint on results_log is satisfied.
                    run_id = str(uuid.uuid4())
                    insert_test_run(run_id, scenario_id,
                                    datetime.now(timezone.utc),
                                    os.getenv("HOSTNAME", "orchestrator"))
    ...
    def get_scenario_end_time(self, scenario_id: str) -> datetime | None:
        """Get the pre-computed end time for a scenario, or None if not set."""
        return self.scenario_end_times.get(scenario_id)
