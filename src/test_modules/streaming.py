...
class StreamingResult:
    ...
    total_rebuffer_time: float      # seconds - total time spent rebuffering
    ...
    latency_sample_count: int       # number of latency samples used to compute jitter
...
def _aggregate_results(results: list[StreamingResult]) -> StreamingResult:
    ...
    # Weighted rebuffer ratio from total rebuffer time / total wall playback time
    total_rebuf = sum(r.total_rebuffer_time for r in results)
    total_wall = sum(r.playback_seconds + r.total_rebuffer_time for r in results)
    agg_rebuffer_ratio = total_rebuf / total_wall if total_wall > 0 else 0.0

    # Pooled standard deviation for jitter across all latency samples
    total_samples = sum(r.latency_sample_count for r in results)
    if total_samples > 1:
        grand_mean = sum(r.latency * r.latency_sample_count for r in results) / total_samples
        pooled_var = sum(
            (r.latency_sample_count - 1) * r.jitter ** 2 +
            r.latency_sample_count * (r.latency - grand_mean) ** 2
            for r in results if r.latency_sample_count > 0
        ) / (total_samples - 1)
        pooled_jitter = pooled_var ** 0.5
    else:
        grand_mean = results[0].latency if results else 0.0
        pooled_jitter = results[0].jitter if results else 0.0
    ...
    return StreamingResult(
        ...
        # Weighted rebuffer ratio
        rebuffer_ratio=agg_rebuffer_ratio,
        ...
        total_rebuffer_time=total_rebuf,
        # Averages for network metrics
        download_speed=sum(r.download_speed for r in results) / n,
        # Sample-weighted latency and pooled jitter
        latency=grand_mean,
        jitter=pooled_jitter,
        latency_sample_count=total_samples,
    )
...
def _run_sequential_streaming(playback_urls: list, headless: bool, disable_cache: bool,
    ...
    with sync_playwright() as p:
        ...
        for url in playback_urls:
            result = _stream_video(context, url, disable_cache)
...
def _get_jellyfin_server_info(server_url: str, api_key: str) -> tuple[str, str]:
    ...
    try:
        ...
        pass
    # Get user ID for the user associated with the API key
    user_id = ""
    try:
        ...
        log_error("streaming", "_get_jellyfin_server_info", e,
                  context=f"Failed to fetch current user from {base}, falling back to /Users")
        # Fallback to first user if /Users/Me not available
        try:
            ...
            log_error("streaming", "_get_jellyfin_server_info", e2,
                      context=f"Failed to fetch user ID from {base}")
            pass
...
def _stream_video(context, url: str, disable_cache: bool = False) -> StreamingResult:
    """Stream a Jellyfin video end-to-end and collect metrics."""
    page = context.new_page()

    # Disable cache via CDP on this page's session (CDP is page-scoped, not context-scoped)
    if disable_cache:
    ...
    # Track requests via network interception (keyed by unique ID, not URL,
    # because HLS/DASH can reuse the same URL for multiple segment requests)
    request_tracking = {"pending": {}}
    _request_counter = {"n": 0}

    def handle_request(request):
        _request_counter["n"] += 1
        req_key = f"{request.url}#{_request_counter['n']}"
        request._tracking_key = req_key
        request_tracking["pending"][req_key] = time.time()

    def handle_response(response):
        req_url = response.url
        req_key = getattr(response.request, '_tracking_key', None)
        rtt_ms = 0.0
        # Track latency for all requests
        if req_key and req_key in request_tracking["pending"]:
            start_time = request_tracking["pending"].pop(req_key)
        ...
        if any(pattern in req_url for pattern in ["/Videos/", "/hls/", ".ts", ".m3u8", "/stream", "/Audio/"]):
            ...
            # Track video-only bytes for accurate bitrate estimation
            try:
                cl = response.headers.get("content-length")
                if cl:
                    metrics["video_bytes_received"] += int(cl)
                # Skip response.body() — downloading large video segments
                # into memory is too expensive; accept undercount when
                # Content-Length is absent (chunked transfer).
            except (ValueError, TypeError):
                pass

    def handle_request_failed(request):
        req_key = getattr(request, '_tracking_key', None)
        if req_key and req_key in request_tracking["pending"]:
            request_tracking["pending"].pop(req_key)
    ...
    # playback_seconds = video.currentTime (media position, excludes buffering waits)
    # Wall-clock playback duration includes rebuffer time
    wall_playback = metrics["playback_seconds"] + metrics["total_rebuffer_time"]

    # Calculate rebuffer ratio
    if wall_playback > 0:
        metrics["rebuffer_ratio"] = metrics["total_rebuffer_time"] / wall_playback

    # Active playback = media position (currentTime already excludes rebuffer waits)
    metrics["active_playback_seconds"] = metrics["playback_seconds"]
    ...
    # Estimate bitrate from video segment bytes and media duration
    if metrics["playback_seconds"] > 0:
        metrics["est_bitrate_bps"] = (metrics["video_bytes_received"] * 8) / metrics["playback_seconds"]

    # Calculate network metrics (download_speed, latency, jitter)
    ...
    if all_latencies:
        ...
        if len(all_latencies) > 1:
            ...
            variance = sum((x - mean) ** 2 for x in all_latencies) / (len(all_latencies) - 1)
    ...
    return StreamingResult(
        ...
        total_rebuffer_time=metrics["total_rebuffer_time"],
        download_speed=metrics["download_speed"],
        latency=metrics["latency"],
        jitter=metrics["jitter"],
        latency_sample_count=len(metrics["all_latencies"]),
    )
...
def _monitor_full_playback(page, metrics: dict) -> None:
    ...
    while True:
        ...
        try:
            ...
            # Track buffer levels (include zero to capture buffer-empty states)
