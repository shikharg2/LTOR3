...
def _start_rtp_capture(pcap_file: str, timeout: int) -> subprocess.Popen | None:
    ...
    try:
        ...
        # Wait for tshark to fully attach to the capture interface
        # before SIPp starts sending traffic.
...
def _parse_rtp_streams(pcap_file: str, media_type: str) -> dict:
    ...
    if audio_streams:
        ...
        # Fix: denominator must be total sent (received + lost), not just received
        metrics["audio_rtp_packet_loss_rate"] = total_lost / (total_pkts + total_lost) if (total_pkts + total_lost) > 0 else 0.0
    ...
    if video_streams:
        ...
        # Fix: denominator must be total sent (received + lost), not just received
        metrics["video_rtp_packet_loss_rate"] = total_lost / (total_pkts + total_lost) if (total_pkts + total_lost) > 0 else 0.0
    ...
    # Fix: sent = received + lost (lost packets were sent but never arrived)
    all_sent_pkts = sum(s["sent_packets"] for s in streams)
    all_sent_bytes = sum(s["sent_bytes"] for s in streams)
    metrics["media_packets_sent"] = float(all_sent_pkts)
    metrics["media_packet_loss"] = float(all_lost)
    # Fix: denominator must be total sent (received + lost), not just received
    metrics["media_packet_loss_rate"] = all_lost / (all_pkts + all_lost) if (all_pkts + all_lost) > 0 else 0.0
    total_bytes = sum(s.get("bytes", 0) for s in streams)
    metrics["media_bytes_received"] = float(total_bytes)
    metrics["media_bytes_sent"] = float(all_sent_bytes)
    total_bitrate = sum(s.get("bitrate_kbps", 0.0) for s in streams)
    metrics["media_rx_bitrate_kbps"] = total_bitrate
    metrics["media_tx_bitrate_kbps"] = sum(
        s["bitrate_kbps"] * s["sent_packets"] / s["packets"] if s["packets"] > 0 else 0.0
        for s in streams
    )
...
def _parse_rtp_stream_table(output: str) -> list[dict]:
    ...
    for line in output.splitlines():
        ...
        try:
            ...
            # Estimate sent-side values (received + lost)
            sent_packets = packets + lost
            sent_bytes = sent_packets * avg_pkt_size
