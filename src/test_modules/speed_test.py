...
def estimate_speed_test_duration(duration: int) -> int:
    """
    Estimate wall-clock seconds for a single-target speed test iteration.

    Covers: 3 iperf3 runs (download, upload, UDP) each with -O 5 omit time,
    2 inter-test sleeps of 5s each, and a ping measurement (~30s worst case).
    Formula: 3 * duration + 25 seconds.
    """
    return 3 * duration + 25
...
def run_speed_test(parameters: dict) -> list[SpeedTestResult]:
    ...
    parallel_streams = parameters.get("parallel_streams", 5)

    results = []
    for url in target_urls:
        host, port = _parse_url(url)
        result = _run_iperf3_test(host, port, duration, parallel_streams)
...
def _run_iperf3_test(host: str, port: int, duration: int, parallel_streams: int = 5) -> SpeedTestResult:
    ...
    download_result = _execute_iperf3(host, port, duration, reverse=True, udp=False, parallel_streams=parallel_streams)
    ...
    upload_result = _execute_iperf3(host, port, duration, reverse=False, udp=False, parallel_streams=parallel_streams)
...
def _execute_iperf3(host: str, port: int, duration: int, reverse: bool, udp: bool = False, parallel_streams: int = 5) -> dict:
    ...
    try:
        ...
        if result.returncode != 0:
            log_error("speed_test", "_execute_iperf3",
                      Exception(f"iperf3 returned exit code {result.returncode}: {result.stderr}"),
                      context=f"host={host}:{port}")
            return {}
...
if __name__ == "__main__":
    params = {
        "target_url":["jwxbbadld-iperf.honeywellaerospace.com:443"],
        "duration" : 10,
        "parallel_streams": 5
    }
